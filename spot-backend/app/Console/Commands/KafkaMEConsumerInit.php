...
class KafkaMEConsumerInit extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'kafka_me:consumer_init';
     ...
     * @var string
     */
    protected $description = 'Command receive request data init from ME';
    ...
    public function handle()
    {
        ...
        $topic = Consts::KAFKA_TOPIC_ME_BE_INIT;
        $handler = new HandlerMEConsumerInit();
...
class HandlerMEConsumerInit {
    ...
    public function __invoke(KafkaConsumerMessage $message)
    {
        ...
        try {
            $type = $data['type'];
			if (in_array($type, ['init-complete'])) {
			...
			if (in_array($type, ['all', 'pair'])) {
				...
				Utils::kafkaProducerME(Consts::KAFKA_TOPIC_ME_INIT, ['type' => "pair-end"]);
			}
				...
				User::with('AccountProfileSetting')
					->chunkById(100, function($members) use($currencies, $userWithdraws, &$sendAccount) {
						...
						if ($users) {
							...
							//send kafka
							Utils::kafkaProducerME(Consts::KAFKA_TOPIC_ME_INIT, $accounts);
							$sendAccount = true;
						}
					});
				if ($sendAccount) {
					Utils::kafkaProducerME(Consts::KAFKA_TOPIC_ME_INIT, ['type' => "account-end"]);
				}
...
//            }
				...
				if ($sendOrder) {
					Utils::kafkaProducerME(Consts::KAFKA_TOPIC_ME_INIT, ['type' => "order-end"]);
				}
			}
        } catch (Exception $e) {
