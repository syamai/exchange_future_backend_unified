...
class RefundPartnerReferral extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'affiliate:partner';
    ...
    public function handle()
    {
        ...
        do {
            $completeTransactions = CompleteTransaction::where('is_calculated_partner_commission', 0)
                ->where('executed_date', Carbon::yesterday())
            ...
            foreach ($completeTransactions as $transaction) {
                DB::beginTransaction();
                try {
                    $this->referralService->addPartnerCommission($transaction);

                    $transaction->is_calculated_partner_commission = 1;
                    $transaction->save();
                    $listSuccess[] = $transaction->id;
                    DB::commit();
                } catch (Exception $e) {
                    DB::rollBack();
                    Log::stack([$channel])->error('RefundPartnerReferral. Failed to calculate partner commission for transaction: ' . $transaction->id);
                    Log::stack([$channel])->error($e);
                    $transaction->is_calculated_partner_commission = 2;
                    $transaction->save();
