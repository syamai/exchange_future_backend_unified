...
class SpotKlineSymbolWorkerCommand extends SpotKlineWorkerCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'spot:kline_worker_symbol {currency} {coin}';
    ...
    public function handle()
    {
        ...
        $this->checkingInterval = env('OP_CHECKING_INTERVAl_KLINE', 3000);
        $this->batchSize = env('OP_KLINE_NUMBER', 20);
        $this->sleepTime = env('OP_KLINE_SLEEP_OTHER', 20);

        if (in_array($this->coin, ['btc'])) {
            $this->sleepTime = env('OP_KLINE_SLEEP_BTC', 1);
        }
    ...
    {
        return "spot_kline_symbol_{$this->currency}_{$this->coin}_prices";
    }

    private function getLastRunKey(): string
    {
        return 'last_run_kline_symbol_' . $this->currency . '_' . $this->coin;
    }
    ...
    {
        $minId = $process->processed_id;
        if ($minId < $this->minId) {
            $minId = $this->minId;
        }
        $query = Price::where('id', '>', $minId)->where(['currency' => $this->currency, 'coin' => $this->coin]);
