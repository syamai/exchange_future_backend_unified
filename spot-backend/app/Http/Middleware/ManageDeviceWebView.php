...
class ManageDeviceWebView
{
    public function handle($request, Closure $next, $guard = null)
    {
        $requestData = request();
        $isWebview = $requestData->get('is_webview');
        $disableVerifyDevice = env('DISABLE_VERIFY_DEVICE', false);

        if (!$disableVerifyDevice && !$isWebview && !$request->header(Consts::API_KEY, null) && Auth::check() && Auth::user()->isValid() && $this->isPassport()) {
