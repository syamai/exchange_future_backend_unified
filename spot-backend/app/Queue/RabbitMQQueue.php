...
class RabbitMQQueue extends Queue implements QueueContract
{
    ...
    public function __construct(AmqpContext $context, array $config)
    {
        ...
        $this->exchangeOptions['arguments'] = isset($this->exchangeOptions['arguments']) ? json_decode($this->exchangeOptions['arguments'], true) : [];
...
//            return;
    ...
    protected function reconnectRabbitMQ()
    {
        logger("RECONNECT RABBITMQ");
        $count = 0;
        while ($count < 3) {
            try {
                $this->context->close();
                $factory = new AmqpConnectionFactory($this->config);
                $this->context = $factory->createContext();
                break;
            } catch (\Exception $e) {
                $count++;
                logger()->error("RECONNECT FAIL TO RABBITMQ ====== $count");
            }
        }
        if ($count >= 3) {
            logger()->info("CAN'T RECONNECT TO RABBITMQ ====> EXIT");
            exit(1);
        }
    }
}
