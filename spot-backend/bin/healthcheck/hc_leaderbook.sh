...
last_history_query="SELECT id FROM margin_balance_history ORDER BY id DESC LIMIT 1;"
last_history_id=`/usr/bin/mysql -h$host -u$username -p$password -P$port $database -s -N -e "$last_history_query"`

last_processed_query="SELECT processed_id FROM processes WHERE \`key\`='update_margin_leaderboard' limit 1;"
...
/usr/bin/aws cloudwatch put-metric-data \
    --metric-name "${env}MarginLeaderboard" \
