...
for symbol in "${symbols[@]}"; do
    ...
    logfilename="MatchingEngine$symbol"
    ...
    tail -n 1000 $logfile | tac > $temp_file

    matching_engine_spot_value=""
    max_interval="0"
    ...
    do
        ...
        if (( $latest_record_time - $unixtime > 60000 )); then
    ...
    /usr/bin/aws cloudwatch put-metric-data --metric-name "$net$symbol" --dimensions InstanceId=$aws_instance  --namespace "MatchingEngine" --value $max_interval
