...
# Spot exchange
last_history_query="SELECT id FROM order_transactions ORDER BY id DESC LIMIT 1;"
last_history_id=`/usr/bin/mysql -h$host -u$username -p$password -P$port $database -s -N -e "$last_history_query"`

last_processed_query="SELECT processed_id FROM processes WHERE \`key\`='update_trading_volume_ranking' limit 1;"
...
/usr/bin/aws cloudwatch put-metric-data \
    --metric-name "${env}SpotTradingVolumeRanking" \
    --dimensions InstanceId=$aws_instance \
    --namespace "SpotExchange" \
    --value $diff

# Margin exchange
last_history_query="SELECT id FROM margin_trades ORDER BY id DESC LIMIT 1;"
last_history_id=`/usr/bin/mysql -h$host -u$username -p$password -P$port $database -s -N -e "$last_history_query"`

last_processed_query="SELECT processed_id FROM margin_processes WHERE \`key\`='update_trading_volume_ranking' limit 1;"
last_processed_id=`/usr/bin/mysql -h$host -u$username -p$password -P$port $database -s -N -e "$last_processed_query"`

diff=$(( $last_history_id - $last_processed_id ))

/usr/bin/aws cloudwatch put-metric-data \
    --metric-name "${env}MarginTradingVolumeRanking" \
    --dimensions InstanceId=$aws_instance \
