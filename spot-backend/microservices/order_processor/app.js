...
subscribeRedisClient.subscribe('StartNewOrderProcessor');

subscribeRedisClient.on('message', (channel, data) => {
    console.log(channel, JSON.stringify(data));
    if (channel === 'StartNewOrderProcessor') {
        const pair = JSON.parse(data);
        console.log('StartNewOrderProcessor', pair);
...
function startProcessor(currency, coin) {
    ...
    console.log('Start processor currency: ' + currency + ', coin: ' + coin + ', currentTime: ' + currentTime);
    setTimeout(function () {
        runCommand(
            'php',
            ['artisan', 'order:process', currency, coin],
            function () {
...
function startProcessorIfNeed(currency, coin, showError, startIfKeyNotFount) {
    var redisKey = 'last_run_' + currency + '_' + coin;
