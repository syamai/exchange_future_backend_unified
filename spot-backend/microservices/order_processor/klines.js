...
var symbols = [];
var checkingInterval = process.env.OP_CHECKING_INTERVAl_KLINE || 3000;
...
function startProcessor(currency, coin) {
    ...
    console.log('Start processor kline currency: ' + currency + ', coin: ' + coin + ', currentTime: ' + currentTime);
    setTimeout(function () {
        runCommand(
            'php',
            ['artisan', 'spot:kline_worker_symbol', currency, coin],
            function () {
...
function startProcessorIfNeed(currency, coin, showError, startIfKeyNotFount) {
    var redisKey = 'last_run_kline_symbol_' + currency + '_' + coin;
    redisClient.get(redisKey, function (error, reply) {
        ...
        if (currentTime > parseInt(checkingInterval) + lastRun) {
            ...
            if (showError) {
                console.error('Processor kline for symbol: ' + coin + '/' + currency + ' already running');
