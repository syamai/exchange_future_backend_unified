...
var symbols = [];
var checkingInterval = process.env.OP_CHECKING_INTERVAl || 30000;
var processOrderPrefix = process.env.PROCESS_ORDER_REQUEST_REDIS_PREFIX || ''
...
subscribeRedisClient.on('message', (channel, data) => {
    ...
    if (channel === 'StartFakeDataProcessor') {
        ...
        startProcessorIfNeed(pair.currency, pair.coin);
    }
});
...
function startProcessor(currency, coin) {
    ...
    console.log('Start processor order request currency: ' + currency + ', coin: ' + coin + ', currentTime: ' + currentTime);
    setTimeout(function () {
        runCommand(
            'php',
            ['artisan', 'order:process_request', currency, coin],
            function () {
...
function startProcessorIfNeed(currency, coin, showError, startIfKeyNotFount) {
    var redisKey = 'process_order_request_' + processOrderPrefix + currency + '_' + coin;
    redisClient.get(redisKey, function (error, reply) {
        ...
        if (currentTime > parseInt(checkingInterval) + lastRun) {
            ...
            if (showError) {
                console.error('Processor for symbol: ' + coin + '/' + currency + ' already running');
